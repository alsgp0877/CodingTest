...
<details>
  ...
  - 코드를 거의 다 구현했는데 왜 안되나 하고 질문창을 보니까 for i in range(s,e+1) 형태로 코드를 짠사람을 봤다. 그래서 혹시나 했더니 ㅋㅋㅋ되었다..
  - 왜 for i in arr[s:e+1]: 이 코드는 안되는거야?.. 일전에 수열과  구간 쿼리 2,3 문제 풀었을때는 됬던 코든데..
  - 알고보니 이번문제는 arr 배열 값 자체에 +1 이라는 연산을 수행했는데 그 값을 또 슬라이싱에 사용하니 원하는 for문이 안나왔던거다
  - 예를 들어 첨에 0 1 2 3 4 였던것이 분기문에 의해 +1 씩 되면서 1 2 4 5 4 이렇게 됬는데 이 변경된 값을 슬라이싱에 사용을 하면 당연히 원하던 슬라이싱 구간이 안나올것임!..
  - 근데 range를 이용하면 그냥 정수값을 사용하기에 원하는 순차가 가능함..ㅠㅠ이전에 2, 3도 값변경이 없엇기에 가능했다
  - arr를 이용해 사용하고 싶으면 tmp 배열을 하나 추가해서 작업하는것이 맞아 보인다
